...
#include <tables/saw_analogue512_int8.h> // sine table for oscillator
...
Oscil <TRIANGLE_VALVE_2048_NUM_CELLS, AUDIO_RATE> aSaw(TRIANGLE_VALVE_2048_DATA);
Oscil <TRIANGLE_VALVE_2048_NUM_CELLS, AUDIO_RATE> aSaw2(TRIANGLE_VALVE_2048_DATA);
Oscil <TRIANGLE_VALVE_2048_NUM_CELLS, AUDIO_RATE> aSaw3(TRIANGLE_VALVE_2048_DATA);
Oscil <SAW_ANALOGUE512_NUM_CELLS, AUDIO_RATE> aChum(SAW_ANALOGUE512_DATA);
Oscil <SAW_ANALOGUE512_NUM_CELLS, AUDIO_RATE> aChum2(SAW_ANALOGUE512_DATA);
Oscil <SAW_ANALOGUE512_NUM_CELLS, AUDIO_RATE> aChum3(SAW_ANALOGUE512_DATA);
...
pixels.begin(); // INITIALIZE NeoPixel strip object (REQUIRED)
  ...
  aChum.setFreq(pitch1);
  aChum2.setFreq(pitch2);
  aChum3.setFreq(pitch3);
...
aChum.setFreq(pitch1 + pitchshift);
if(knob1state == 0)
{aSaw2.setFreq(pitch2 + pitchshift);
aChum2.setFreq(pitch2 + pitchshift);
}
else
{aSaw2.setFreq(pitch1 + knob1value + pitchshift);
aChum2.setFreq(pitch1 + knob1value + pitchshift);
}
if(knob2state == 0)
{aSaw3.setFreq(pitch3 + pitchshift);
aChum3.setFreq(pitch3 + pitchshift);
}
else
{aSaw3.setFreq(pitch1 + knob2value + pitchshift);
aChum3.setFreq(pitch1 + knob2value + pitchshift);
}
...
{if(midisource == 1)
{return MonoOutput::from8Bit(aChum.next()*gain1/273+aChum2.next()*gain2/273+aChum3.next()*gain3/273);}
else
{return MonoOutput::from8Bit(aChum.next()*gain1b+aChum2.next()*gain2b+aChum3.next()*gain3b);}
}}
